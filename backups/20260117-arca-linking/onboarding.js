...
const stepOneSchema = Joi.object({
  ...
  arcaRef: Joi.string().optional() // Anonymous Arca ID to link
});
...
router.post('/step1', onboardingLimiter, async (req, res) => {
  try {
    ...
    const { firstName, lastName, email, phone, password, dateOfBirth, arcaRef } = value;
    ...
    const user = new User({
      ...
      signupSource: 'web',
      linkedArcaIds: arcaRef ? [arcaRef] : [], // Link anonymous Arca contributions
      isVerified: false,
      isActive: true,
    ...
    console.log(`📝 Step 1 completed for: ${maskEmail(email)} (pending email verification)`);
...
router.post('/step3', async (req, res) => {
  try {
    ...
    // Check if email is verified before allowing deposit
    if (!user.isVerified) {
    ...
    console.log(`🎉 Onboarding completed for: ${maskEmail(user.email)} - ${tokensToMint} GAMBINO tokens`);
...
router.post('/resend-verification', async (req, res) => {
  try {
    ...
    // Rate limiting: Check if resend attempted within last 2 minutes
