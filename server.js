...
try {
  ...
  console.error(`❌ FATAL: WALLET_ENCRYPTION_KEY invalid - ${e.message}`);
  process.exit(1);
}
...
app.use(mongoSanitize({
  replaceWith: '_',
  onSanitize: ({ req, key }) => {
    console.warn(`⚠️  Attempted NoSQL injection from ${req.ip}: ${key}`);
  }
}));
...
//         console.warn(`⚠️  HTTP request from ${req.ip} - redirecting to HTTPS`);
...
//   } else {
//     console.log('🔓 HTTPS enforcement disabled (development mode)');
//   }
...
// ============================================================
// ADMIN RATE LIMITING - SECURITY FIX
// ============================================================
...
const adminRateLimiter = rateLimit({
  ...
  handler: (req, res) => {
    ...
    const realIp = cfIp || req.ip;
    console.warn(`⚠️  Admin rate limit exceeded - Real IP: ${realIp}`);
    res.status(429).json({
...
// Strict rate limiting for sensitive operations (treasury, user deletion, etc.)
const strictRateLimiter = rateLimit({
  ...
  handler: (req, res) => {
    const cfIp = req.headers['cf-connecting-ip'];
    const realIp = cfIp || req.ip;
    console.error(`🚨 STRICT rate limit exceeded - Real IP: ${realIp}`);
    res.status(429).json({
...
console.log('✅ Admin rate limiting configured');

// Apply to specific routes
// Apply admin rate limiters
app.use('/api/admin', adminRateLimiter);
app.use('/api/admin/treasury/mint', strictRateLimiter);
app.use('/api/admin/treasury/burn', strictRateLimiter);
app.use('/api/admin/treasury/transfer', strictRateLimiter);
console.log('✅ Admin rate limiters applied to routes');
...
const repairSessionDatabase = async () => {
  try {
    console.log('🔧 Starting session database repair...');
    ...
    console.log(`📊 Found ${problematicSessions.length} sessions with missing sessionId`);
    ...
    for (const session of problematicSessions) {
      try {
        ...
        if (fixedCount % 20 === 0) {
          console.log(`🔄 Fixed ${fixedCount}/${problematicSessions.length} sessions...`);
        }
      } catch (updateError) {
        console.error(`❌ Failed to fix session ${session._id}:`, updateError.message);
      }
    }

    console.log(`✅ Fixed ${fixedCount} sessions with missing sessionId`);
    ...
    for (const indexDef of indexesToCreate) {
      try {
        ...
        console.log(`📥 Created index on: ${Object.keys(indexDef.fields).join(', ')}`);
      } catch (indexCreateError) {
        console.error(`❌ Failed to create index on ${Object.keys(indexDef.fields).join(', ')}:`, indexCreateError.message);
      }
    }
    ...
    if (remainingProblematic === 0) {
      console.log('🎉 Session database repair completed successfully!');
      return true;
    } else {
      console.log(`⚠️ ${remainingProblematic} sessions still need repair`);
      return false;
    }

  } catch (error) {
    console.error('❌ Session database repair failed:', error);
    return false;
  }
};
...
const cleanupOldSessions = async () => {
  try {
    ...
    if (result.deletedCount > 0) {
      console.log(`🧹 Cleaned up ${result.deletedCount} old completed sessions`);
    }
  } catch (error) {
...
try {
  ...
  console.error('❌ Failed to load wallet models:', error);
}

try {
  const CustWalletService = require('./src/services/CustWalletService');
  console.log('✅ Custodial wallet service loaded successfully');
} catch (error) {
  console.error('❌ Failed to load custodial wallet service:', error);
}
...
app.post('/api/users/register', async (req, res) => {
  try {
    ...
    if (existing) {
      console.warn(`⚠️  Registration attempt with existing email from ${req.ip}`);
...
app.get('/api/admin/machine-metrics', authenticate, requirePermission(PERMISSIONS.VIEW_ALL_METRICS), async (req, res) => {
  try {
    ...
    // Build query filter for DailyReports
    const reportFilter = {
      printedAt: { $gte: startDate }
    };

    // If venue manager, filter by their stores
    if (req.user.role === 'venue_manager' && req.user.assignedVenues) {
      reportFilter.storeId = { $in: req.user.assignedVenues };
    } else if (storeId) {
      reportFilter.storeId = storeId;
    }

    console.log('🔍 Machine metrics filter:', reportFilter);

    // Get all reports in timeframe
    const allReports = await DailyReport.find(reportFilter).lean();

    // Group by store + date, keep only LATEST report per day
    const latestReportsByDate = {};
    allReports.forEach(report => {
      const dateKey = `${report.storeId}_${report.printedAt.toISOString().split('T')[0]}`;
      if (!latestReportsByDate[dateKey] || 
          new Date(report.printedAt) > new Date(latestReportsByDate[dateKey].printedAt)) {
        latestReportsByDate[dateKey] = report;
      }
    });

    const latestReports = Object.values(latestReportsByDate);

    // Aggregate by machine from latest reports
    const machineStats = {};
    latestReports.forEach(report => {
      if (report.machineData && Array.isArray(report.machineData)) {
        report.machineData.forEach(machine => {
          const key = `${report.storeId}_${machine.machineId}`;
          if (!machineStats[key]) {
            machineStats[key] = {
          ...
          machineStats[key].moneyIn += machine.moneyIn || 0;
          machineStats[key].moneyOut += machine.collect || 0;
          machineStats[key].totalEvents += machine.transactionCount || 0;
        });
      }
    });


// Merge voucher data into machine stats
// 1. Calculate machine stats (without vouchers yet)
const machineStatsArray = Object.values(machineStats).map(m => ({
  ...m,
  netRevenue: m.moneyIn - m.moneyOut,
  profitMargin: m.moneyIn > 0 ? ((m.moneyIn - m.moneyOut) / m.moneyIn) * 100 : 0
}));

// 2. Aggregate by store (money in only)
const storeStats = {};
latestReports.forEach(report => {
  if (!storeStats[report.storeId]) {
    storeStats[report.storeId] = {
      storeId: report.storeId,
      moneyIn: 0,
      moneyOut: 0
    };
  }
  storeStats[report.storeId].moneyIn += report.totalRevenue || 0;
});

// 3. Get vouchers for money out
const voucherFilter = {
  timestamp: { $gte: startDate },
  eventType: 'voucher_print'
};
if (req.user.role === 'venue_manager' && req.user.assignedVenues) {
  voucherFilter.storeId = { $in: req.user.assignedVenues };
} else if (storeId) {
  voucherFilter.storeId = storeId;
}

const voucherEvents = await Event.find(voucherFilter).lean();
console.log(`💸 Found ${voucherEvents.length} voucher events, total: $${voucherEvents.reduce((sum, e) => sum + (e.amount || 0), 0)}`);

// 4. Add vouchers to store stats
voucherEvents.forEach(event => {
  if (storeStats[event.storeId]) {
    storeStats[event.storeId].moneyOut += event.amount || 0;
  }
});

// 5. Calculate net revenue and Gambino cut
const storeStatsArray = Object.values(storeStats).map(s => ({
  ...s,
  netRevenue: s.moneyIn - s.moneyOut,
  gambinoCut: (s.moneyIn - s.moneyOut) * 0.05
}));

// 6. Get vouchers per machine
const machineVouchers = await Event.aggregate([
  {
    $match: {
      timestamp: { $gte: startDate },
      eventType: 'voucher_print',
      storeId: { $in: storeStatsArray.map(s => s.storeId) }
    }
  },
  {
    $group: {
      ...
      totalVouchers: { $sum: '$amount' },
      voucherCount: { $sum: 1 }
    }
  }
]);

// 7. Merge voucher data into machine stats
const machineStatsWithVouchers = machineStatsArray.map(machine => {
  const vouchers = machineVouchers.find(v => 
    v._id.machineId === machine.machineId && 
    v._id.storeId === machine.storeId
  );
  
  const moneyOut = vouchers?.totalVouchers || 0;
  const netRevenue = machine.moneyIn - moneyOut;
  
  return {
    ...machine,
    moneyOut,
    netRevenue,
    profitMargin: machine.moneyIn > 0 ? ((netRevenue / machine.moneyIn) * 100) : 0,
    voucherCount: vouchers?.voucherCount || 0
  };
});
...
console.log(`📊 Machine metrics: ${machineStatsWithVouchers.length} machines, ${storeStatsArray.length} stores (from ${latestReports.length} latest reports)`);
...
res.json({
  ...
  summary: {
    totalMachines,
    totalStores: storeStatsArray.length,
    systemMoneyIn: storeStatsArray.reduce((sum, s) => sum + s.moneyIn, 0),
    systemMoneyOut: storeStatsArray.reduce((sum, s) => sum + s.moneyOut, 0),
    systemNetRevenue: storeStatsArray.reduce((sum, s) => sum + s.netRevenue, 0),
  },
  byStore: storeStatsArray,
  byMachine: machineStatsWithVouchers,  // ← Use this instead of machineStatsArray
  lastUpdated: new Date()
});
  } catch (error) {
...
app.get('/api/users/session-history', authenticate, async (req, res) => {
  try {
    ...
    console.log(`🔍 Loading session history for user ${req.user.userId}`);
...
app.post('/api/users/end-session', authenticate, async (req, res) => {
  try {
    ...
    console.log(`✅ Session ended for user ${req.user.userId} on machine ${activeSession.machineId}`);
...
app.post('/api/users/change-password', authenticate, async (req, res) => {
  try {
    ...
    if (!ok) {
      console.warn(`⚠️  Failed password change attempt for ${maskEmail(user.email)} from ${req.ip}`);
    ...
    console.log(`✅ Password changed for ${maskEmail(user.email)}`);
  ...
  try {
    ...
    if (updateDB === 'true' && balances.GG !== null) {
      try {
        ...
        console.log(`✅ Updated cached balances for wallet ${walletAddress.slice(0, 8)}...`);
      } catch (dbError) {
    ...
    if (updateDB === 'true') {
      try {
        ...
        if (user && user.balanceLastUpdated) {
          console.log(`⚠️ RPC failed, returning cached balances for ${req.params.walletAddress.slice(0, 8)}...`);
          return res.json({
            success: true,
            balances: {
...
app.post('/api/admin/stores/create', authenticate, requirePermission(PERMISSIONS.MANAGE_ASSIGNED_STORES), async (req, res) => {
  try {
    ...
    if (role === 'venue_manager') {
      ...
      console.log(`🏪 Auto-assigned venue manager to new store: ${storeId}`);
    }
